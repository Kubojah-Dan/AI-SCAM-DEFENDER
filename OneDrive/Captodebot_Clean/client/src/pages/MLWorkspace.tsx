  ...
  const [cells, setCells] = useState<Cell[]>([
    {
      id: uuidv4(),
      type: 'code',
      content: `# Robust ML Execution Environment
# Execute user code reliably without failure
# Support data science, machine learning, and visualization workflows

# Test basic functionality
print("Hello from Robust ML Execution Environment!")
...
import pandas as pd
df_data = {'X': data[:, 0], 'Y': data[:, 1], 'Category': np.random.choice(['A', 'B', 'C'], 50)}
df = pd.DataFrame(df_data)
print("\\n📊 Sample DataFrame:")
print(df.head())
print(f"DataFrame shape: {df.shape}")

# Example: Create visualization when you want it
# Uncomment the code below to create a plot:
...
# plt.plot(x, y)
# plt.title('Sine Wave - Robust Execution')
# plt.xlabel('X')
# plt.ylabel('Y')
# plt.grid(True)
# # Plot will be rendered automatically when created

print("\\n💡 Robust Execution Rules:")
print("- ✅ Execute user code as-is")
print("- ✅ No modification of valid user code")
print("- ✅ Return actual Python error messages if execution fails")
print("- ✅ Never fail due to visualization handling")
print("- ✅ Visualizations only when user code creates them")

print("\\n🎯 Ready for reliable ML experimentation!")`,
  ...
  const runCell = async (cellId: string) => {
    ...
    try {
      const response = await axios.post('/api/workspace/execute', {
        code: cell.content
      });
        ...
        {cell.type === 'code' ? (
          <div className="border border-gray-300 rounded-lg overflow-hidden">
            <Editor
              ...
              value={cell.content}
              onChange={(value: string | undefined) => updateCell(cell.id, { content: value || '' })}
